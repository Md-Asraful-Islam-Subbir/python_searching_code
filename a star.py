...
def AStarSearch():
    ...
    while True:
        ...
        if closed[-1][0] == 'G':  # cng 'G'
            break
#Iterates over the neighbors of the current node.If a neighbor is already in the closed list, it skips to the next neighbor.
...
#Backtracks from the goal node to the start node
    ...
    while par[goal] != goal:
        ...
        goal = par[goal]
#Completes the optimal sequence list and prints it
    optimal_sequence.append(goal)
    optimal_sequence.reverse()
    print(optimal_sequence)
#Returns the lists of visited nodes
